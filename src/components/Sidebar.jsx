        ...
        activeMenu && (
          ...
          <div className="flex justify-between items-center">
            ...
            <TooltipComponent content="Menu" position="BottomCenter">
              <button className="text-xl text-gray-500 rounded-full p-3 hover:bg-light-gray mt-4 block xl:hidden"onClick={()=>setActiveMenu((prevActiveMenu)=> !prevActiveMenu)}>
                <MdOutlineCancel/>
              </button>
            </TooltipComponent>
          </div>
          <div className="mt-10">
            {
              links.map((item)=>(
                <div key={item.title}>
                   ...
                   {
                    item.links.map((link)=>(
                      <NavLink to={`/${link.namelink}`} key={link.namelink} onClick={handleCloseSidebar} 
                      // style={({isActive})=> isActive ? {background:currentColor} : {background:"transparent"}}
                      style={
